...
def add_page_number_with_header(canvas, _, team, image_path,confidential, title):
    ...
    image_y = 775  # 60 units from the top margin
    ...
    canvas.drawCentredString((width / 2) + 190, 20, text)
...
def home_page():
    ...
    if uploaded_files:
        ...
        if len(uploaded_dfs) > 1:
            ...
            merged_df = merged_df.replace(['nan'], '').fillna('')
        ...
        if selected_columns:
            ...
            if st.button("Generate PDF",disabled=button_disabled):
                ...
                buffer = BytesIO()
                doc = SimpleDocTemplate(buffer, pagesize=landscape(A3), leftMargin=0.4 * inch,
                                        rightMargin=0.4 * inch, topMargin=1.2 * inch, bottomMargin= 0.8* inch)
                ...
                # Logo
                logo_path = 'path/logo-cimb.JPG'  # logo location
                ...
                for row_page in range(num_row_pages):
                    ...
                    for col_page in range(num_col_pages):
                        ...
                        table.hAlign = 'LEFT'  # Align the table to the left
                ...
                for i, group in enumerate(grouped_note_lines):
                    ...
                    left_column = Table([[line] for line in left_content],
                                        colWidths=[6.5 * inch]) if i == 0 else Paragraph("", no_gap_style)



                    row = [left_column, Table([[line] for line in note_paragraphs])]
